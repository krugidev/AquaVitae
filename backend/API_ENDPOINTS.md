...
| POST | `/recuperar-password` `{ identificador }` | — | ✅ **implementado** — código de 6 dígitos em `utilizador_password_reset` (15 min de validade), 202 sempre (não revela se a conta existe) — **desde 2026-09-25 o 202 traz um corpo `{ validadeSegundos: 900, novoPedidoEmSegundos: 60 }`, igual para qualquer conta (existente ou não: não revela nada), com que a app arranca o temporizador do ecrã do código e o "pedir novo código"** —. Envio real por email ainda por fazer — por agora só regista em log (`PasswordResetService`, combinado adiar o email). **`identificador` = username OU email, como no login** (sem distinguir maiúsculas, com `trim`; **mudou em 2026-09-22**, antes era `email`; o corpo antigo com `email` dá 400) |
...
| POST | `/api/users/me/apagar` `{ password }` | ✅ **implementado** (2026-09-25, validado ao vivo) — **apagar a própria conta** (o popup "Apagar conta"; a Google Play exige-o a apps com registo). Confirma com a password da conta: **403** "Password incorreta." (não 401 — a app tenta renovar a sessão a cada 401), 400 se em branco, 409 numa conta de administrador. Apaga, numa só transação, garrafas e caves, wishlist, favoritos, "provadas", reviews (o trigger recalcula o rating das bebidas), preferências, códigos de recuperação e sessões, e por fim o utilizador. **Os cliques em links de compra ficam anónimos** (`utilizador_id` a `NULL`: já não são dados pessoais). 204; o token do utilizador deixa de servir (401). `ContaService` + `ApagarContaRepository`; 5 testes |
...
- ✅ **Feito na app em 2026-09-23 (fatia 3a: popup de detalhe da bebida)** — `ReviewModels.kt`, `AquaVitaeApi.kt`,
  ...
  - `GET /api/users/me/reviews` **passou a ser usado na app em 2026-09-25** (fatia 10, "As minhas reviews"); `POST /api/auth/recuperar-password` passou a devolver `CodigoInfo` e `POST /api/users/me/apagar` é novo (a app usa-os no "Conta e segurança").
