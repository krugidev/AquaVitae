...
`testDebugUnitTest` (**117**, ver `android/README.md`). Corri o
...
verificado em 2026-09-24) — serve de "website" nos pedidos a outras redes de afiliados; continua a não se mexer no repo/domínio. **Redesenhada a 2026-09-25** (screenshots, descrição, termos, privacidade e contacto — ver "Feed da Daisycon", abaixo, para o porquê e como se atualiza); há uma 2.ª cópia em `https://krugidev.github.io/aquavitae/` (minúsculas, para a Daisycon).
...
no repositório. **Media verificada a 2026-09-25 (pela metatag; o método do ficheiro dava "Connection to the remote server failed" e a metatag passou — a metatag e o ficheiro ficam publicados no `krugidev.github.io`, na raiz e em `/aquavitae/`). Falta a aprovação da media pela Daisycon ("pode demorar alguns dias"; avisam por notificação) e depois candidatar-se à campanha da Decantalo, ler os termos e só então escolher a media no painel do feed.** Como se chegou lá:  o ficheiro que a Daisycon deu (`7197e10404415e4.html`) foi publicado em `docs/` da `main` (commit `40f7241`, servido em `https://krugidev.github.io/AquaVitae/7197e10404415e4.html`, conteúdo idêntico); **a Daisycon guarda o URL da media sempre em minúsculas (`https://krugidev.github.io/aquavitae/`) e o GitHub Pages distingue maiúsculas no caminho do repositório (404). Resolvido a 2026-09-25 com um 2.º repositório público, `krugidev/krugidev.github.io` (site de utilizador), que serve `/aquavitae/` com uma cópia da landing page (com `canonical` para `/AquaVitae/`) e o mesmo ficheiro de verificação; o `/AquaVitae/` da Awin não mudou.** Se um dia se mexer na landing page, mexer nas duas cópias (ou trocar a cópia por um redirecionamento). **Media reprovada e landing page refeita (2026-09-25):** a Daisycon recusou a media ("could not see your media": pede uma descrição
boa, um domínio válido e contactos). A landing page foi refeita (commit `e0b80fd` na `main`, cópia no `krugidev.github.io` em `b53e07c`): hero com 2 ecrãs reais da app, "O que é a AquaVitae", "Como funciona"
(4 passos), galeria de 7 screenshots (`docs/assets/screens/`, de dados de demonstração — a conta e os dados criados para as capturas foram apagados), funcionalidades, estado do projeto
("Lançamento previsto: em breve" — **data por definir**), transparência (afiliados, 18+, consumo responsável), contacto **`aquavitaerecovery@gmail.com`** visível no cabeçalho, no bloco de contacto e no rodapé, e as
páginas **`termos.html`** e **`privacidade.html`** (**rascunhos escritos por mim a partir do que a app faz — não são aconselhamento jurídico; convém o utilizador rever, e depois reutilizar os termos no `legal/termos.txt` da app, que
ainda é Lorem ipsum**). Só HTML/CSS estáticos, sem cookies, analytics nem recursos de terceiros; a metatag de verificação está em todas as páginas. **Como se atualiza:** editar em `docs/` (na `main`, por um worktree: `git worktree add --detach <pasta> origin/main`,
commit, `git push origin HEAD:main`, `git worktree remove`), copiar `index.html`, `privacidade.html`, `termos.html` e `assets/` para `aquavitae/` do clone de `krugidev.github.io` e fazer push — os `href`/`src` são relativos, por isso servem nos dois URLs. **Falta:** o utilizador abrir um ticket
de suporte na Daisycon a pedir nova inspeção da media 432814 (texto pronto na conversa), esperar a resposta, e só então candidatar-se à campanha da Decantalo. Rever antes os **termos da campanha** (comparadores/apps, uso de imagens e descrições, comissão, cookie) e a `retalhista` da Decantalo (rede "Daisycon").
...
1. **Ronda de bugs (combinada com o utilizador em 2026-09-24, a seguir à fatia 6)**, antes de começar a meter bebidas reais.
   Candidatos já conhecidos, para não os perder: (o "1 reviews" sem singular já foi corrigido — `textoReviews`, commit `88cc473`, no cartão, no popup e em Favoritos);
